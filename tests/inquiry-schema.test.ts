...
describe("parseInquiry", () => {
  ...
  it("honeypot(website) 채워지면 silent-reject", () => {
    const r = parseInquiry({ type: "idea", name: "bot", contact: "", body: "spam spam spam spam", website: "http://spam" });
  ...
  it("contact 생략 가능 (기본값 빈 문자열)", () => {
    const r = parseInquiry({ type: "try", name: "친구", body: "에이전트 한번 써보고 싶어요!", website: "" });
    expect(r.ok).toBe(true);
